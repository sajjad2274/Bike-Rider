    ...
    public class CustomAdmob : MonoBehaviour, ICustomAds
    {
        ...
        private void LoadBanner(BannerPosition position, BannerType bannerType)
        {
            ...
            switch (position)
            {
                ...
                case BannerPosition.BOOTOM_LEFT:
                    if (bannerType == BannerType.SmartBanner)
                    {
                        banner = new BannerView(bannerId, AdSize.MediumRectangle, AdPosition.BottomLeft);
                    }
                    else
                    {
                        banner = new BannerView(bannerId, AdSize.MediumRectangle, AdPosition.BottomLeft);
                ...
                case BannerPosition.BOOTOM_RIGHT:
                    if (bannerType == BannerType.SmartBanner)
                    {
                        banner = new BannerView(bannerId, AdSize.MediumRectangle, AdPosition.BottomRight);
                    }
                    else
                    {
                        banner = new BannerView(bannerId, AdSize.MediumRectangle, AdPosition.BottomRight);
        ...
        private void InterstitialFailed(object sender, AdFailedToLoadEventArgs e)
        {
            ...
            if (debug)
            {
                ...
                ScreenWriter.Write("Admob Interstitial -> Load error string: " + loadAdError.ToString());
        ...
        private void OnAdFailedToShow(object sender, AdErrorEventArgs e)
        {
            if (debug)
            {
                Debug.Log("Admob OnAdFailedToShow " + e.AdError.GetMessage());
                ScreenWriter.Write("Admob OnAdFailedToShow " + e.AdError.GetMessage());
            }
        }
        ...
        private void RewardedVideoFailed(object sender, AdFailedToLoadEventArgs args)
        {
            ...
            if (debug)
            {
                ScreenWriter.Write("Admob Rewarded Video Failed ");
